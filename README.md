...
主图下方有一个「高级别 · 多周期」副图面板，显示主图的**更高一级周期**缠论结构
（大级别看全局），与主图共享同一个 K 线缓存（键 = 交易所:品种:周期），不重复请求。

- **周期跟随**：联动开启（默认）时，主图周期变化副图自动切换为更高一级周期
  （1m→5m→15m→30m→1h→4h→1d→1w；1w 到顶）。工具栏「🔗 联动」按钮开关。
- **副图独立**：联动关闭后，副图可通过右上角下拉框独立选择周期，主图变化不再影响。
- **点击联动**：点击副图某根高级别 K 线，主图自动滚动到该 K 线对应的时间区间
  （从大周期看全局 → 点大周期 K 线看小周期细节）。
- **视觉联动**：副图上用半透明色块高亮主图当前可见的时间范围；主图上画出副图
  （高级别）K 线边界虚线，标出大级别位置。
- **数据隔离**：主图与副图各自拥有独立的 klinecharts 实例、缠论状态机与显示开关。
  klinecharts 的自定义指标回调通过 `indicator.id` 反查所属视图（`viewByIndicatorId`），
  两个图表互不干扰。副图默认只显示「笔 / 线段 / 线段中枢 / 背驰」核心高级别信号。
- 副图面板 CSS 常量：主图高度 = 100% − 268px，副图面板固定 268px（`style.css`）。
